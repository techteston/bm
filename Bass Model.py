...
def bass_model(p, q, M, periods=500):
    """
    ...
    F_t = expit((p + q) * t - np.log(q / p))
    
    # Calculate F(t-1)
    t_1 = np.arange(0, periods)
    F_t_1 = expit((p + q) * t_1 - np.log(q / p))
    
    # Calculate S(t)
    S_t = M * (F_t - F_t_1)
    
    # Calculate cumulative sales
    cumulative_sales = np.cumsum(S_t)
...
def bass_model2(p, q, M, periods=50):
    """
    Implementation of the Bass model for product adoption.
...
import plotly.express as px
fig = px.line(x=df["Period"], y=df["Cumulative Sales"],color=px.Constant("Cumulative Sales"))
fig.add_bar(x=df["Period"], y=df["Sales in Period"], name="Sales in Period")
fig.show()
